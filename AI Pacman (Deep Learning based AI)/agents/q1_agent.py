...
class Q1Agent(ValueEstimationAgent):
        ...
        if self.values is None:
            ...
            for _ in range(self.iterations):
                ...
                for state in possible_states:
                    ...
                    # deal with special case if terminal
                    if(self.MDP.isTerminal(state)):
                        new_values[state] = self.MDP.getReward(state,"DONE",state)
                        continue
                    ...
                    # if an action was determined, update accordingly
                    if(maxVal!=-float('inf')):
    ...
    def computeQValueFromValues(self, state: tuple, action):
        ...
        # get transition positions and probabilities
        stateProbList = self.MDP.getTransitionStatesAndProbs(state, action)
        # calculate value
        qVal = 0

        for (newState, prob) in stateProbList:
    ...
    def computePolicyFromValues(self, state: tuple):
        ...
        return actionList[np.argmax([self.computeQValueFromValues(state,action) for action in actionList])]