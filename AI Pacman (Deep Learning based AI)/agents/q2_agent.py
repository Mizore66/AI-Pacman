...
class Q2Agent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state: GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        return max(self.getLegalActions(state), key=lambda action: self.getQValue(state, action)) if len(self.getLegalActions(state)) > 0 else None
    ...
    def epsilonGreedyActionSelection(self, state: GameState):
        ...
        if util.flipCoin(self.epsilon):
            return random.choice(self.getLegalActions(state)) if len(self.getLegalActions(state)) > 0 else None
        else:
            return self.computeActionFromQValues(state)
    ...
    def update(self, state: GameState, action, nextState: GameState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        if self.learningQvalues:
            self.Q_values[state.getPacmanPosition()[0], state.getPacmanPosition()[1], self.getActionIndex(action)] = (1 - self.alpha) * self.getQValue(state, action) + self.alpha * (reward + self.discount * self.computeValueFromQValues(nextState))
